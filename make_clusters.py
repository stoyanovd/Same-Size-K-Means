...
def calc_ans_inside(n_clusters, array_id_lon_lat):
    X = np.array([[p['lon'], p['lat']] for p in array_id_lon_lat])
...
def draw_plot(n_clusters, array_id_lon_lat):
    ...
    plt.savefig('ans_geo_clusters.png')
...
def main():
    ...
    required_named.add_argument('--draw_plot', type=int,
                                help='draw plot or not draw (into file "ans_geo_clusters.png"). Put 1 or 0.',
                                required=True)
    ...
    calc_ans_inside(args.n_clusters, array_id_lon_lat)

    if args.draw_plot == 1:
        draw_plot(args.n_clusters, array_id_lon_lat)

    json.dump(array_id_lon_lat, open(args.output_json_file, 'w', encoding='utf-8'))
